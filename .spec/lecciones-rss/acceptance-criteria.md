...
| AC-009 | REQ-007, REQ-008, NFR-003, NFR-014 | Repetir el tick programado en la misma fecha local omite sin API ni cambio de fuente; reintentar el mismo run manual también omite. Un run manual nuevo puede generar una extra. |
...
| AC-024 | NFR-008, histórico reemplazado por AC-042 y AC-045 | La versión original rechazaba lotes mayores que `maxLessonsPerRun` y permitía una llamada por curso/run; v0.9 procesa hasta el límite y conserva el resto como deuda. |
...
| AC-028 | Histórico, reemplazado por AC-035 | La migración a `gemini-3.8-flash` conservó el contrato JSON en pruebas locales, pero los pedidos reales registrados devolvieron HTTP 503 y no publicaron una lección. |
