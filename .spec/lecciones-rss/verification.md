...
Enmienda v1.0 (2026-09-25): el run `36137099953` confirmó dos respuestas completas pero menores de 900 palabras y un HTTP 429 en el tercer intento; la entrega de Música clásica siguió pendiente. Las pruebas locales de la nueva estrategia comprueban ampliación guiada, máximo de tres llamadas, ausencia de escritura ante dos borradores cortos y 429, deuda conservada, hash del prompt efectivo, límite de contexto y registro seguro de `Retry-After`/cuota. `npm run validate` validó cuatro cursos, `npm run typecheck` pasó, `npm test` pasó (32 pruebas en 3 archivos) y `git diff --check` no reportó errores. El éxito con Gemini real y la clasificación exacta de su límite de cuota permanecen sin verificar hasta un nuevo run posterior al push.
